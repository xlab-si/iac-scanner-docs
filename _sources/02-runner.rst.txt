...
+-------------------------------+----------------------------+----------------------------+----------------------------+
| `Steampunk Scanner`_          | Ansible                    | no                         | yes                        |
+-------------------------------+----------------------------+----------------------------+----------------------------+
...
**Steampunk Scanner** (developed by `XLAB Steampunk`_) is a quality scanner for Ansible playbooks, where you can scan
the playbook and get an instant quality score with tips on how to improve it.
The `steampunk-scanner`_ CLI enables the use of Ansible scanner from the console with the ability to scan Ansible task
files, playbooks, roles and collections (see `Steampunk Scanner check`_).

+-------------------------+----------------------------+
| Check ID (from the API) | ``steampunk-scanner``      |
+-------------------------+----------------------------+
| Enabled (by default)    | no                         |
+-------------------------+----------------------------+
| Configured (by default) | no                         |
+-------------------------+----------------------------+
| Documentation           | `Steampunk Scanner docs`_  |
+-------------------------+----------------------------+
...
.. admonition:: Configuration options for `/checks/{check_name}/configure`_ API endpoint
    ...
    :Secret:
        ...
        Accepts username (e-mail) and user token (inputted as `<username>:<token>`) needed to authenticate to Steampunk
        Scanner API.
        The API token has to be first generated by downloading `steampunk-scanner`_ package and the by running
        ``steampunk-scanner authenticate`` CLI command that will register the new user and send the token to the
        user's e-mail.
...
.. admonition:: Configuration options for `/checks/{check_name}/configure`_ API endpoint
    ...
    :Secret:
        ...
        Accepts an optional user token to authenticate to `SonarQube`_ or `SonarCloud`_.
...
.. _Steampunk Scanner check: https://github.com/xlab-steampunk/steampunk-scanner-cli
.. _XLAB Steampunk: https://steampunk.si/
.. _Steampunk Scanner docs: https://scanner.steampunk.si/
.. _steampunk-scanner: https://pypi.org/project/steampunk-scanner/
